...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('id, pha_id, name, fullname, nickname, prefix, role, profile_image, salary_number, must_change_password, created_at')
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { userId, fullname, nickname, salary_number, role } = body;
